...
import { Cache, EventPayload, OnEvent } from '../../fundamentals';
...
export class DocContentService {
  ...
  @OnEvent('snapshot.updatesPushed')
  async markDocContentCacheStale({ workspaceId, id }: EventPayload<'snapshot.updatesPushed'>) {
    const key =
      workspaceId === id
